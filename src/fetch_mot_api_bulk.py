...
def get_access_token() -> str:
    if not TOKEN_URL or not CLIENT_ID or not CLIENT_SECRET:
        sys.exit(
            "ERROR: Set MOT_CLIENT_ID, MOT_CLIENT_SECRET, MOT_TOKEN_URL "
...
# Download a single file from a presigned S3 URL 
def download_file(url: str, dest: Path) -> Path:
    """Download from a presigned S3 URL, skipping if dest already exists."""
...
def main():
    print("=" * 60)
    print("MOT History API — Bulk + Delta fetcher")
    print("=" * 60)
